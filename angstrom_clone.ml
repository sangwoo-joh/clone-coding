...
module Parser = struct
  module State = struct
    ...
    and 'a partial =
      { committed : int
      ; continue : Bytes.t -> off:int -> len:int -> More.t -> 'a t
      }
  end
  ...
  let rec to_exported_state = function
    | State.Partial { committed; continue } ->
      Exported_state.Partial
        { committed
        ; continue =
            (fun bs ~off ~len more -> to_exported_state (continue bs ~off ~len more))
        }
  ...
  module Choice = struct
      ...
      { run =
          (fun input pos more fail succ ->
            let fail' input' pos' more' marks msg =
              fail input' pos' more' (mark :: marks) msg
            in
            p.run input pos more fail' succ)
      }
    ;;
    ...
    let ( <|> ) p q =
      { run =
          (fun input pos more fail succ ->
            let fail' input' pos' more' marks msg =
              if pos < Input.parser_committed_bytes input'
              then fail input' pos' more marks msg
